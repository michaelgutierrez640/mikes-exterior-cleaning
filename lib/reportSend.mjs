  ...
  const delivery = extras.delivery
    ? {
        ...
        recipient: extras.delivery.recipient
          ? (() => {
              const email = String(extras.delivery.recipient)
...
export async function runReport(type, options = {}) {
  ...
  try {
    ...
    let from
    try {
      ;({ apiKey, to, from } = assertEmailConfig())
    } catch (err) {
      const delivery = await saveDeliveryRecord({
        ...
        failureReason: err?.message || 'Email configuration missing',
        subject,
      ...
      return publicRunResult(
        ...
        },
        {
          delivery,
          diagnostics,
          error: err?.message || 'Email configuration missing',
        },
      )
    }

    const diagnosticsWithFrom = {
      ...diagnostics,
      fromAddress: from,
      fromMatchesExpected: from.toLowerCase().includes('reports@reports.mikesexteriorcleaning.com'),
    }
...
function summarizeCronResult(entry) {
 ...
 * Always persists a compact cron status so missed runs are diagnosable.
 */
export async function runScheduledReports(now = new Date()) {
  ...
  if (due.weekly) {
    if (!settings.weeklyEnabled) {
      ...
      results.push({
        type: 'weekly',
        ...(await runReport('weekly', { range: due.weekly, priorRange: getPriorWeekRange(due.weekly) })),
      })
    }
  } else {
  ...
  const weekly = results.find((r) => r.type === 'weekly')
  ...
  await saveCronRunStatus({
    ...
    weekly: summarizeCronResult(weekly),
    monthly: summarizeCronResult(monthly),
  }).catch(() => {})
