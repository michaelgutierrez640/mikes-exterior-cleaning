...
/**
 * Parse a required 1–5 star rating.
 ...
 * Display consent is implied by the submit notice (no checkbox).
...
export function validateReviewIngest(input = {}) {
  ...
  const ratingResult = parseStarRating(input.rating)

  if (!name) return { ok: false, error: 'Name is required', status: 400 }
  if (!ratingResult.ok) return { ok: false, error: ratingResult.error, status: 400 }
  ...
  return {
    ...
    data: {
      ...
      // Implied by submit notice on /review
