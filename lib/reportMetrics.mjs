...
export async function buildPeriodMetrics(range, preloaded = {}) {
  ...
  // Sessions (total visitors): prefer sessionId; fall back to visitorId when older events lack sessions.
  const totalVisitors = uniq(
    pageViews.map((e) => e.sessionId || e.visitorId),
  ).length
  ...
  return {
    ...
    totalVisitors: metric(totalVisitors, analyticsReady),
