...
/**
 * Derive appointmentStatus from lead pipeline status (UI no longer edits this).
 * Preserves requested/none for early statuses; does not wipe unrelated values.
 */
export function deriveAppointmentStatusForLeadStatus(status, existing = {}) {
  const canonical = getCanonicalStatus(status)
  if (canonical === 'Booked') return 'confirmed'
  if (canonical === 'Completed') return 'completed'
  if (canonical === 'Lost') {
    const hadAppointment = Boolean(
      existing.appointmentDate ||
        existing.appointmentConfirmedAt ||
        ['requested', 'confirmed', 'completed'].includes(existing.appointmentStatus),
    )
    return hadAppointment ? 'cancelled' : existing.appointmentStatus || DEFAULT_APPOINTMENT_STATUS
  }
  return existing.appointmentStatus || DEFAULT_APPOINTMENT_STATUS
}
...
export function filterLeads(leads, {
  ...
  let filtered = (leads || []).filter((lead) => {
    ...
    if (query) {
      const hay = [lead.name, lead.phone, lead.email, lead.service, lead.city, lead.message, lead.followUpNote, lead.internalNotes]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
...
export function validateLeadAdminUpdate(input = {}, existing = {}) {
  ...
  if (hasOwn(input, 'lostReason')) {
    patch.lostReason = trimStr(input.lostReason, MAX_LOST_REASON_LENGTH)
  }
...
export async function updateLead(id, input = {}) {
  ...
  if (patch.status !== undefined) {
    ...
    if (patch.status !== current || existing.status !== patch.status) {
      ...
      // Appointment status is derived from pipeline status (not edited in admin UI).
      existing.appointmentStatus = deriveAppointmentStatusForLeadStatus(patch.status, existing)
      existing.appointmentTimezone = existing.appointmentTimezone || DEFAULT_APPOINTMENT_TIMEZONE
      ...
      if (patch.status === 'Booked') {
        ...
        if (!existing.paymentStatus) {
          existing.paymentStatus = DEFAULT_PAYMENT_STATUS
        }
      }
    }
  }
  ...
  if (patch.lostReason !== undefined) {
    existing.lostReason = patch.lostReason
    changed = true
  }
  ...
  // Keep derived appointment status in sync when pipeline is Booked/Completed/Lost.
  const canonicalStatus = getCanonicalStatus(existing.status)
  if (['Booked', 'Completed', 'Lost'].includes(canonicalStatus)) {
    const derived = deriveAppointmentStatusForLeadStatus(canonicalStatus, existing)
    if (existing.appointmentStatus !== derived) {
      existing.appointmentStatus = derived
      changed = true
    }
  }
