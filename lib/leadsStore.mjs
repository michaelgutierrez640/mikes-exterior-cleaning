...
/**
 * Mark matching leads opted out of customer SMS (Twilio STOP).
 * Preserves historical consent timestamp; sets smsOptedOut for send gates.
 */
export async function applySmsOptOutByPhone(phone, { at = nowIso() } = {}) {
  const redis = getAnalyticsRedis()
  if (!redis) {
    const err = new Error('Leads storage not configured')
    err.status = 503
...
/**
 * Clear opt-out when customer texts START (optional re-subscribe).
 * Does not invent consent — smsConsent must already be true or a new form opt-in.
 */
export async function applySmsStartByPhone(phone, { at = nowIso() } = {}) {
  const redis = getAnalyticsRedis()
  if (!redis) {
    const err = new Error('Leads storage not configured')
    err.status = 503
