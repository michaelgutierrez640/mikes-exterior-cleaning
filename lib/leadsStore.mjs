...
import {
  ...
  normalizeLeadPhotos,
  presentLeadPhotosForClient,
} from './leadPhotos.mjs'
import { deleteBlobUrls } from './projectBlobCleanup.mjs'
...
export function presentLead(lead) {
  ...
  return {
    ...
    // Never expose raw Blob URLs to Admin/browser clients.
    photos: presentLeadPhotosForClient(Array.isArray(lead.photos) ? lead.photos : []),
...
export function validateLeadIngest(input = {}) {
  ...
  // Reject photo payloads while customer uploads are disabled (public store cannot keep them private).
  if (photosResult.value.length && !isLeadPhotoUploadEnabled()) {
    return {
      ok: false,
      error: 'Customer photo uploads are disabled until a private Blob store is configured',
      status: 503,
    }
  }
...
/**
 * Permanently delete one lead by id: Redis key + index membership + attached Blob objects.
...
export async function permanentlyDeleteLead(id) {
  ...
  const photoUrls = (Array.isArray(existing.photos) ? existing.photos : [])
    .map((p) => p?.url)
    .filter(Boolean)
  ...
  let blobCleanup = { deleted: 0, errors: [] }
  if (photoUrls.length) {
    const leadToken = String(process.env.LEAD_PHOTOS_BLOB_READ_WRITE_TOKEN || '').trim()
    blobCleanup = await deleteBlobUrls(photoUrls, {
      token: leadToken || process.env.BLOB_READ_WRITE_TOKEN,
    })
    if (blobCleanup.errors?.length) {
      console.error('[leads] blob cleanup errors after permanent delete', {
        id: leadId,
  ...
  return {
    ...
    photosDeleted: blobCleanup.deleted || 0,
  }
}
