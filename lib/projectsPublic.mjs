    ...
    .map((p, index) => {
      ...
      const variants =
        p.variants && typeof p.variants === 'object'
          ? {
              ...(p.variants.thumb && /^https:\/\//i.test(p.variants.thumb)
                ? { thumb: p.variants.thumb }
                : {}),
              ...(p.variants.card && /^https:\/\//i.test(p.variants.card)
                ? { card: p.variants.card }
                : {}),
              ...(p.variants.gallery && /^https:\/\//i.test(p.variants.gallery)
                ? { gallery: p.variants.gallery }
                : {}),
            }
          : null
      const blurDataUrl =
        typeof p.blurDataUrl === 'string' && p.blurDataUrl.startsWith('data:image/')
          ? p.blurDataUrl
          : null
      return {
        url,
        ...
        blurDataUrl,
        variants: variants && Object.keys(variants).length ? variants : null,
      }
    })
    .filter(Boolean)
...
/**
 * Flatten every photo from published projects for the public Our Work gallery.
 * Newest published project first; photo sortOrder preserved within each project.
 * Drafts / unpublished / invalid records never appear.
 */
export async function listPublicGalleryItems({ limitProjects = 100 } = {}) {
  const published = await listProjects('published')
  ...
  const items = []
  for (const project of projects.slice(0, limitProjects)) {
