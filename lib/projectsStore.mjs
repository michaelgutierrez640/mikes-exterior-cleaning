...
function normalizeVariants(raw) {
  ...
  const gallery = String(raw.gallery || '').trim()
  const card = String(raw.card || '').trim()
  const thumb = String(raw.thumb || '').trim()
  const variants = {}
  if (/^https:\/\//i.test(gallery)) variants.gallery = gallery
  if (/^https:\/\//i.test(card)) variants.card = card
  if (/^https:\/\//i.test(thumb)) variants.thumb = thumb
...
function normalizePhoto(raw, index = 0) {
  ...
  const width = Number.isFinite(Number(raw.width)) ? Math.round(Number(raw.width)) : null
  const height = Number.isFinite(Number(raw.height)) ? Math.round(Number(raw.height)) : null
  const blurDataUrl = String(raw.blurDataUrl || '').trim()
  const safeBlur =
    blurDataUrl.startsWith('data:image/') && blurDataUrl.length <= 12000 ? blurDataUrl : null
