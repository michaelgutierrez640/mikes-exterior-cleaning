...
export function validateProjectInput(input = {}, { partial = false } = {}) {
  ...
  if (!partial || input.photos !== undefined) {
    ...
    // First-time create/publish still requires at least one photo.
    // Edits may clear photos (published projects keep a public placeholder).
    if (!partial && (data.status === 'published' || input.status === 'published') && photos.length === 0) {
...
export async function updateProject(id, input) {
  ...
  // First publish (draft → published) still requires photos. Clearing photos on an
  // already-published job is allowed so admins can remove the last image safely.
  const firstPublishing = existing.status !== 'published' && next.status === 'published'
  if (firstPublishing && (!next.photos || next.photos.length === 0)) {
