...
function buildHtml({ title, rangeLabel, payload, summaryLines, adminUrl, businessName, type }) {
  ...
  const kpi = [
    ...
    ['New CRM leads', fmt(payload.totalLeads), fmtCmp(c.totalLeads)],
    ['Lead conversion', fmtPctRate(payload.conversionRate), fmtCmp(c.conversionRate)],
    ['Phone clicks', fmt(payload.phoneClicks), fmtCmp(c.phoneClicks)],
  ...
  const summaryHtml = summaryLines.map((line) => `<li style="margin:0 0 8px;color:${GRAY};font-size:15px;line-height:1.5;">${esc(line)}</li>`).join('')
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:640px;background:#ffffff;border-radius:16px;overflow:hidden;">
        ...
        <tr><td style="padding:24px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${kpiHtml}</table>
          ...
          <p style="margin:20px 0 0;font-size:12px;color:#9ca3af;line-height:1.5;">Private aggregate report for ${esc(businessName)}. No customer names, emails, phone numbers, or notes are included.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function buildText({ title, rangeLabel, payload, summaryLines, adminUrl, businessName, type }) {
  ...
  const lines = [
    ...
    `- Quote starts: ${fmt(payload.instantQuoteStarts)} | ${fmtCmp(c.instantQuoteStarts)}`,
    `- Quote completions: ${fmt(payload.instantQuoteCompletions)} | ${fmtCmp(c.instantQuoteCompletions)}`,
    `- Contact forms: ${fmt(payload.contactFormSubmissions)}`,
    `- Booking requests: ${fmt(payload.bookingRequests)}`,
    `- Projects published: ${fmt(payload.projectsPublished)} | ${fmtCmp(c.projectsPublished)}`,
    '',
    'SUMMARY',
