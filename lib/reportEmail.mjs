 ...
 */
import { sanitizeEmailBody } from './reportBody.mjs'
...
function esc(value) {
  return sanitizeEmailBody(value)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
...
export function buildReportEmail(opts) {
  ...
  // Final sanitize: strip control chars that can make Gmail render a blank body.
  return {
    subject: sanitizeEmailBody(subject),
    html: sanitizeEmailBody(html),
    text: sanitizeEmailBody(text),
  }
}
...
function buildHtml({ title, rangeLabel, payload, summaryLines, adminUrl, businessName, type }) {
  ...
  const kpi = [
    ['Total visitors (sessions)', fmt(payload.totalVisitors), fmtCmp(c.totalVisitors)],
    ['Unique visitors', fmt(payload.uniqueVisitors), fmtCmp(c.uniqueVisitors)],
    ['Page views', fmt(payload.pageViews), fmtCmp(c.pageViews)],
    ['Completed jobs published', fmt(payload.projectsPublished), fmtCmp(c.projectsPublished)],
    ['Quote / CRM leads', fmt(payload.totalLeads), fmtCmp(c.totalLeads)],
