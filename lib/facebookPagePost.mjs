...
/**
 * Caption body is the saved Short job notes with any project URLs removed.
 * The canonical project URL is appended by finalizeFacebookCaption.
 */
export function buildFacebookCaptionBody(project) {
  return stripProjectUrlsFromText(project?.notes || '')
}

/**
 * Build the final Facebook caption from Short job notes + one canonical URL:
 *
 *   [Short job notes]
 *   https://www.mikesexteriorcleaning.com/projects/{saved-slug}
 *
 * Always uses the saved job notes (not a separate caption field). Strips any
 * placeholder/duplicate project links from notes before appending the URL.
 */
export function finalizeFacebookCaption(project, _rawCaption = '') {
  ...
  const body = buildFacebookCaptionBody(project)
  const caption = sanitizeFacebookCaption(body ? `${body}\n${projectUrl}` : projectUrl)
  ...
  // Require notes + URL on separate lines when notes are present.
  if (body && !caption.includes(`\n${projectUrl}`)) {
    return { ok: false, error: 'Facebook caption must place the project link on its own line after the job notes' }
  }
...
export function buildDefaultFacebookCaption(project) {
  ...
  if (!finalized.ok) {
    const body = buildFacebookCaptionBody(project)
    return body ? sanitizeFacebookCaption(body) : ''
  }
  return finalized.caption
}

export function sanitizeFacebookCaption(value) {
  return sanitizePublicText(value, { maxLength: FACEBOOK_CAPTION_MAX, preserveNewlines: true })
}
 ...
 * @param {object} [deps]
 */
export async function maybePostProjectToFacebook(project, _opts = {}, deps = {}) {
  ...
  if (!SERVICE_SLUGS.includes(String(project.service || '')) || !CITY_SLUGS.includes(String(project.city || ''))) {
    const failed = await saveState(projectId, {
      ...
      facebookCaption: sanitizeFacebookCaption(buildFacebookCaptionBody(project)),
    })
    return { project: failed, skipped: false, status: 'failed', error: failed.facebookPostError }
  }

  // Always rebuild from Short job notes + saved slug. Never post placeholders or guessed URLs.
  const finalized = finalizeFacebookCaption(project)
  if (!finalized.ok) {
    const failed = await saveState(projectId, {
      facebookPostStatus: 'failed',
