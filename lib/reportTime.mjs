...
/**
 * Vercel Cron schedule in vercel.json: "0 15 * * *" (daily 15:00 UTC).
 * That is 8:00 AM PDT / 7:00 AM PST (America/Los_Angeles).
 * Weekly email is only sent when that run falls on a Monday in Pacific time.
 */
export const WEEKLY_CRON_UTC = '0 15 * * *'
export const WEEKLY_CRON_UTC_HOUR = 15

/**
 * Human-readable next weekly arrival in America/Los_Angeles.
 */
export function describeNextWeeklySchedule(now = new Date()) {
  const dateKey = nextWeeklySendDate(now)
  // Cron fires at 15:00 UTC on that calendar day (UTC). Map to Pacific wall time.
  const utcInstant = new Date(`${dateKey}T${String(WEEKLY_CRON_UTC_HOUR).padStart(2, '0')}:00:00.000Z`)
  ...
  return {
    ...
    dateKey,
    localLabel,
    timeOfDayLabel: hourOnly,
    weekdayLabel: 'Monday',
    cronUtc: WEEKLY_CRON_UTC,
    windowNote:
      'Vercel Hobby may invoke the job anytime during that UTC hour (about a 60-minute window).',
    summary: `Every Monday at about ${hourOnly} (${REPORT_TZ}). Next: ${localLabel}.`,
  }
}
